...
class Tokens {
    constructor(password, salt) {
        this.key = crypto.pbkdf2Sync(password, salt, 1001, 32, 'sha256');
    }
    ...
    encode(config) {
        ...
        var cipher = crypto.createCipheriv(algo, this.key, iv);
    ...
    decode(token) {
        try {
            ...
            var decipher = crypto.createDecipheriv(algo, this.key, iv);
